# InterfaceGUI/gui_transmissor.py
...
class TransmissorGUI(ttk.Frame):
    def __init__(self, master):
        ...
        self.msg_var = tk.StringVar(value="Ola mundo! 03/07/2025")
        self.enquadramento_var = tk.StringVar(value='Bit Stuffing (Flags)')
        self.mod_digital_var = tk.StringVar(value='NRZ-Polar')
        self.mod_portadora_var = tk.StringVar(value='ASK')
        self.detecao_erro_var = tk.StringVar(value='CRC-32')
        self.correcao_erro_var = tk.StringVar(value='Hamming')
        self.taxa_erros_var = tk.DoubleVar(value=0.01)
    ...
    def _create_widgets(self):
        ...
        # Controle de Taxa de Erros
        ttk.Label(config_frame, text="Taxa de Erros no Canal:").grid(row=6, column=0, sticky="w", padx=5, pady=(10,0))
        error_scale = ttk.Scale(config_frame, from_=0.0, to_=0.1, orient=tk.HORIZONTAL, variable=self.taxa_erros_var, command=lambda v: self.error_label.config(text=f"{float(v):.3f}"))
        ...
        try:
            while not self.update_queue.empty():
                ...
                if msg_type == 'status':
                    ...
                    pass # Logs de console são ignorados pela GUI do transmissor
